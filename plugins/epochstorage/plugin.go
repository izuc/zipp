...
func configure(plugin *node.Plugin) {
	...
	deps.NotarizationMgr.Events.MeshTreeInserted.Attach(event.NewClosure(func(event *notarization.MeshTreeUpdatedEvent) {
	...
	deps.NotarizationMgr.Events.MeshTreeRemoved.Attach(event.NewClosure(func(event *notarization.MeshTreeUpdatedEvent) {
...
func GetEpochBlockIDs(ei epoch.Index) (blockIDs []mesh_old.BlockID) {
...
func insertBlockIntoEpoch(ei epoch.Index, blkID mesh_old.BlockID) error {
	blockStore, err := baseStore.WithRealm(append([]byte{database.PrefixEpochsStorage, prefixBlockIDs}, ei.Bytes()...))
	if err != nil {
		panic(err)
	}
...
func removeBlockFromEpoch(ei epoch.Index, blkID mesh_old.BlockID) error {
	blockStore, err := baseStore.WithRealm(append([]byte{database.PrefixEpochsStorage, prefixBlockIDs}, ei.Bytes()...))
	if err != nil {
		panic(err)
	}
...
func saveEpochVotersWeight(block *mesh_old.Block) {
	voter := identity.NewID(block.IssuerPublicKey())
	activeWeights, _ := deps.Mesh.WeightProvider.WeightsOfRelevantVoters()

...
import (
	...
	db_pkg "github.com/izuc/zipp/packages/node/database"
...
func configureEvents() {
	...
	deps.Mesh.Ledger.Events.TransactionAccepted.Attach(onTransactionAcceptedClosure)
	// mana.Events().Revoked.Attach(onRevokeEventClosure)
}

func onTransactionAccepted(transactionID utxo.TransactionID) {
	deps.Mesh.Ledger.Storage.CachedTransaction(transactionID).Consume(func(transaction utxo.Transaction) {
...
func gatherInputInfos(inputs devnetvm.Inputs) (totalAmount float64, inputInfos []mana.InputInfo) {
	...
	for _, input := range inputs {
		...
		deps.Mesh.Ledger.Storage.CachedOutput(outputID).Consume(func(o utxo.Output) {
			...
			deps.Mesh.Ledger.Storage.CachedOutputMetadata(outputID).Consume(func(metadata *ledger.OutputMetadata) {
	...
	if err := daemon.BackgroundWorker("Mana", func(ctx context.Context) {
		...
		if !readStoredManaVectors() {
			...
			if Parameters.Snapshot.File != "" {
				...
				if err := snapshot.LoadSnapshot(
					...
					utxoStatesConsumer,
					epochDiffsConsumer,
					deps.Mesh.WeightProvider.LoadActiveNodes,
				); err != nil {
		...
		for {
			...
			case <-ctx.Done():
				...
				deps.Mesh.Ledger.Events.TransactionAccepted.Detach(onTransactionAcceptedClosure)
...
func QueryAllowed() (allowed bool) {
	...
	// return deps.Mesh.Bootstrapped() || debuggingEnabled\

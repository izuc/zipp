...
var (
	...
	parentsCountPerType      = make(map[mesh_old.ParentsType]uint64)
...
// BlockTips returns the actual number of tips in the block mesh.
...
func ParentCountPerType() map[mesh_old.ParentsType]uint64 {
	parentsCountPerTypeMutex.RLock()
	defer parentsCountPerTypeMutex.RUnlock()

	// copy the original map
	clone := make(map[mesh_old.ParentsType]uint64)
...
func increasePerParentType(c mesh_old.ParentsType) {
...
func measureBlockTips() {
	blockTips.Store(uint64(deps.Mesh.TipManager.TipCount()))
}
...
func measureRequestQueueSize() {
	size := int64(deps.Mesh.Requester.RequestQueueSize())
	requestQueueSize.Store(size)
}

func measureInitialDBStats() {
	dbStatsResult := deps.Mesh.Storage.DBStats()

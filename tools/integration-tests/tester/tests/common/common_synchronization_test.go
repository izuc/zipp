...
func TestCommonSynchronization(t *testing.T) {
	...
	time.Sleep(newPeer.Config().BlockLayer.MeshTimeWindow)
	log.Println("Issuing blocks... done")
...
func TestFirewall(t *testing.T) {
	...
	}, func(peerIndex int, peerMaster bool, cfg config.ZIPP) config.ZIPP {
		if peerIndex == 0 {
...
func createNewPeerConfig(t *testing.T, snapshotInfo framework.SnapshotInfo, peerIndex int) config.ZIPP {
	...
	conf.BlockLayer.MeshTimeWindow = 30 * time.Second
	return conf
}

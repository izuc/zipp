...
func NewRateSetter(mesh *Mesh) *RateSetter {
	rateSetter := &RateSetter{
		...
		self:                mesh.Options.Identity.ID(),
		issuingQueue:        schedulerutils.NewNodeQueue(mesh.Options.Identity.ID()),
		issueChan:           make(chan *Block),
		ownRate:             atomic.NewFloat64(0),
		pauseUpdates:        0,
		initialPauseUpdates: uint(mesh.Options.RateSetterParams.RateSettingPause / mesh.Scheduler.Rate()),
		maxRate:             float64(time.Second / mesh.Scheduler.Rate()),
...
func (r *RateSetter) Setup() {
	if !r.mesh.Options.RateSetterParams.Enabled {
		r.mesh.BlockFactory.Events.BlockConstructed.Attach(event.NewClosure(func(event *BlockConstructedEvent) {
	...
	r.mesh.Scheduler.Events.BlockScheduled.Attach(event.NewClosure(func(_ *BlockScheduledEvent) {
		if r.pauseUpdates > 0 {
			r.pauseUpdates--
			return
		}
		if r.issuingQueue.Size()+r.mesh.Scheduler.NodeQueueSize(selfLocalIdentity.ID()) > 0 {
			r.rateSetting()
		}
	}))
}
...
func (r *RateSetter) Estimate() time.Duration {
	...
	if r.pauseUpdates > 0 {
		pauseUpdate = time.Duration(float64(r.pauseUpdates)) * r.mesh.Scheduler.Rate()
	}
	// dummy estimate
...
func (r *RateSetter) rateSetting() {
	...
	ownMana := math.Max(r.mesh.Options.SchedulerParams.AccessManaMapRetrieverFunc()[r.self], MinMana)
	maxManaValue := lo.Max(append(lo.Values(r.mesh.Options.SchedulerParams.AccessManaMapRetrieverFunc()), MinMana)...)
	totalMana := r.mesh.Options.SchedulerParams.TotalAccessManaRetrieveFunc()

	ownRate := r.ownRate.Load()

	// TODO: make sure not to issue or scheduled blocks older than TSC
	if float64(r.mesh.Scheduler.NodeQueueSize(r.self)) > math.Max(Wmin, Wmax*ownMana/maxManaValue) {
...
func (r *RateSetter) initializeInitialRate() {
	if r.mesh.Options.RateSetterParams.Initial > 0.0 {
		r.ownRate.Store(r.mesh.Options.RateSetterParams.Initial)
	} else {
		ownMana := math.Max(r.mesh.Options.SchedulerParams.AccessManaMapRetrieverFunc()[r.mesh.Options.Identity.ID()], MinMana)
		totalMana := r.mesh.Options.SchedulerParams.TotalAccessManaRetrieveFunc()

...
func TestScenario_1(t *testing.T) {
	...
	mesh := NewTestMesh(WithConflictDAGOptions(conflictdag.WithMergeToMaster(false)))
	defer mesh.Shutdown()
	...
	for _, alias := range []string{"Block1", "Block2", "Block3", "Block4", "Block5", "Block6", "Block7", "Block8", "Block9"} {
		fmt.Println(alias, lo.PanicOnErr(mesh.Booker.BlockConflictIDs(testFramework.Block(alias).ID())))
		mesh.Storage.BlockMetadata(testFramework.Block(alias).ID()).Consume(func(blockMetadata *BlockMetadata) {
...
func TestObjectiveInvalidity(t *testing.T) {
	...
	// ISSUE Block5
	{
		...
		mesh.Storage.BlockMetadata(blk.ID()).Consume(func(blockMetadata *BlockMetadata) {
...
func checkMarkers(t *testing.T, testFramework *BlockTestFramework, expectedMarkers map[string]*markers.Markers) {
	for blockID, expectedMarkersOfBlock := range expectedMarkers {
		assert.True(t, testFramework.mesh.Storage.BlockMetadata(testFramework.Block(blockID).ID()).Consume(func(blockMetadata *BlockMetadata) {
		...
		if expectedMarkersOfBlock.Size() == 1 {
			...
			mappedBlockIDOfMarker := testFramework.mesh.Booker.MarkersManager.BlockID(currentMarker)
			...
			assert.True(t, testFramework.mesh.Storage.BlockMetadata(mappedBlockIDOfMarker).Consume(func(blockMetadata *BlockMetadata) {
...
func checkConflictIDs(t *testing.T, testFramework *BlockTestFramework, expectedConflictIDs map[string]*set.AdvancedSet[utxo.TransactionID]) {
	for blockID, blockExpectedConflictIDs := range expectedConflictIDs {
		...
		retrievedConflictIDs, errRetrieve := testFramework.mesh.Booker.BlockConflictIDs(testFramework.Block(blockID).ID())
...
func checkNormalizedConflictIDsContained(t *testing.T, testFramework *BlockTestFramework, expectedContainedConflictIDs map[string]*set.AdvancedSet[utxo.TransactionID]) {
	for blockID, blockExpectedConflictIDs := range expectedContainedConflictIDs {
		...
		for it := retrievedConflictIDs.Iterator(); it.HasNext(); {
			testFramework.mesh.Ledger.ConflictDAG.Storage.CachedConflict(it.Next()).Consume(func(b *conflictdag.Conflict[utxo.TransactionID, utxo.OutputID]) {

...
import (
	...
	"github.com/izuc/zipp.foundation/core/generics/thresholdmap"
...
func BenchmarkApprovalWeightManager_ProcessBlock_Conflicts(b *testing.B) {
	...
	mesh := NewTestMesh(ApprovalWeights(weightProvider))
	defer mesh.Shutdown()
	approvalWeightManager := mesh.ApprovalWeightManager
	...
	totalMarkers := 10000
	{
		...
		for i := uint32(1); i < uint32(totalMarkers); i++ {
			if previousMarker == nil {
				previousMarker, _ = mesh.Booker.MarkersManager.Manager.InheritStructureDetails(nil, increaseIndexCallback)
				continue
			}

			previousMarker, _ = mesh.Booker.MarkersManager.Manager.InheritStructureDetails([]*markers.StructureDetails{previousMarker}, increaseIndexCallback)
		}
	}
...
func TestApprovalWeightManager_updateConflictVoters(t *testing.T) {
	...
	mesh := NewTestMesh(ApprovalWeights(weightProvider), WithConflictDAGOptions(conflictdag.WithMergeToMaster(false)))
	defer mesh.Shutdown()
	approvalWeightManager := mesh.ApprovalWeightManager
	...
	createConflict(mesh, "Conflict 1", conflictIDs, set.NewAdvancedSet[utxo.TransactionID](), resourceIDs["Conflict 1"])
	createConflict(mesh, "Conflict 2", conflictIDs, set.NewAdvancedSet[utxo.TransactionID](), resourceIDs["Conflict 1"])
	createConflict(mesh, "Conflict 3", conflictIDs, set.NewAdvancedSet[utxo.TransactionID](), resourceIDs["Conflict 2"])
	createConflict(mesh, "Conflict 4", conflictIDs, set.NewAdvancedSet[utxo.TransactionID](), resourceIDs["Conflict 2"])

	createConflict(mesh, "Conflict 1.1", conflictIDs, conflictIDs["Conflict 1"], resourceIDs["Conflict 3"])
	createConflict(mesh, "Conflict 1.2", conflictIDs, conflictIDs["Conflict 1"], resourceIDs["Conflict 3"])
	createConflict(mesh, "Conflict 1.3", conflictIDs, conflictIDs["Conflict 1"], resourceIDs["Conflict 3"])

	createConflict(mesh, "Conflict 4.1", conflictIDs, conflictIDs["Conflict 4"], resourceIDs["Conflict 4"])
	createConflict(mesh, "Conflict 4.2", conflictIDs, conflictIDs["Conflict 4"], resourceIDs["Conflict 4"])

	createConflict(mesh, "Conflict 4.1.1", conflictIDs, conflictIDs["Conflict 4.1"], resourceIDs["Conflict 5"])
	createConflict(mesh, "Conflict 4.1.2", conflictIDs, conflictIDs["Conflict 4.1"], resourceIDs["Conflict 5"])
	...
	// statement 2: "Conflict 4.1.2"
	{
		...
		mesh.Storage.BlockMetadata(block.ID()).Consume(func(blockMetadata *BlockMetadata) {
...
func TestApprovalWeightManager_updateSequenceVoters(t *testing.T) {
	...
	// build markers DAG
	{
		markersMap["0,1"], _ = mesh.Booker.MarkersManager.Manager.InheritStructureDetails(nil, increaseIndexCallback)
		markersMap["0,2"], _ = mesh.Booker.MarkersManager.Manager.InheritStructureDetails([]*markers.StructureDetails{markersMap["0,1"]}, increaseIndexCallback)
		markersMap["0,3"], _ = mesh.Booker.MarkersManager.Manager.InheritStructureDetails([]*markers.StructureDetails{markersMap["0,2"]}, increaseIndexCallback)
		markersMap["0,4"], _ = mesh.Booker.MarkersManager.Manager.InheritStructureDetails([]*markers.StructureDetails{markersMap["0,3"]}, increaseIndexCallback)

		markersMap["0,1"].SetPastMarkerGap(50)
		markersMap["1,2"], _ = mesh.Booker.MarkersManager.Manager.InheritStructureDetails([]*markers.StructureDetails{markersMap["0,1"]}, increaseIndexCallback)
		markersMap["1,3"], _ = mesh.Booker.MarkersManager.Manager.InheritStructureDetails([]*markers.StructureDetails{markersMap["1,2"]}, increaseIndexCallback)
		markersMap["1,4"], _ = mesh.Booker.MarkersManager.Manager.InheritStructureDetails([]*markers.StructureDetails{markersMap["1,3"]}, increaseIndexCallback)
		markersMap["1,5"], _ = mesh.Booker.MarkersManager.Manager.InheritStructureDetails([]*markers.StructureDetails{markersMap["1,4"]}, increaseIndexCallback)

		markersMap["0,3"].SetPastMarkerGap(50)
		markersMap["1,4"].SetPastMarkerGap(50)
		markersMap["2,5"], _ = mesh.Booker.MarkersManager.Manager.InheritStructureDetails([]*markers.StructureDetails{markersMap["0,3"], markersMap["1,4"]}, increaseIndexCallback)
		markersMap["2,6"], _ = mesh.Booker.MarkersManager.Manager.InheritStructureDetails([]*markers.StructureDetails{markersMap["0,4"], markersMap["2,5"]}, increaseIndexCallback)
		markersMap["2,7"], _ = mesh.Booker.MarkersManager.Manager.InheritStructureDetails([]*markers.StructureDetails{markersMap["2,6"]}, increaseIndexCallback)
		markersMap["2,8"], _ = mesh.Booker.MarkersManager.Manager.InheritStructureDetails([]*markers.StructureDetails{markersMap["2,7"]}, increaseIndexCallback)

		markersMap["2,7"].SetPastMarkerGap(50)
		markersMap["3,8"], _ = mesh.Booker.MarkersManager.Manager.InheritStructureDetails([]*markers.StructureDetails{markersMap["2,7"]}, increaseIndexCallback)
		markersMap["1,4"].SetPastMarkerGap(50)
		markersMap["4,8"], _ = mesh.Booker.MarkersManager.Manager.InheritStructureDetails([]*markers.StructureDetails{markersMap["2,7"], markersMap["1,4"]}, increaseIndexCallback)
	}
...
func TestAggregatedConflictApproval(t *testing.T) {
	...
	testFramework := NewBlockTestFramework(mesh, WithGenesisOutput("G1", 500), WithGenesisOutput("G2", 500))
	...
	// ISSUE Block6
	{
		...
		_, err := mesh.Booker.BlockConflictIDs(testFramework.Block("Block6").ID())
		require.NoError(t, err)
	}
	...
	// ISSUE Block8
	{
		...
		_, err := mesh.Booker.BlockConflictIDs(testFramework.Block("Block8").ID())
		require.NoError(t, err)
	}
}
...
func TestOutOfOrderStatements(t *testing.T) {
	...
	mesh := NewTestMesh(ApprovalWeights(weightProvider), WithConflictDAGOptions(conflictdag.WithMergeToMaster(false)))
	mesh.Booker.MarkersManager.Options.MaxPastMarkerDistance = 3

	mesh.Setup()
	testEventMock := NewEventMock(t, mesh.ApprovalWeightManager)
	testFramework := NewBlockTestFramework(mesh, WithGenesisOutput("A", 500), WithGenesisOutput("B", 500))
...
func approveMarkers(approvalWeightManager *ApprovalWeightManager, voter *identity.Identity, markersToApprove ...markers.Marker) (block *Block) {
	...
	approvalWeightManager.mesh.Storage.StoreBlock(block)
	approvalWeightManager.mesh.Storage.BlockMetadata(block.ID()).Consume(func(blockMetadata *BlockMetadata) {
...
func createConflict(mesh *Mesh, conflictAlias string, conflictIDs map[string]*set.AdvancedSet[utxo.TransactionID], parentConflictIDs *set.AdvancedSet[utxo.TransactionID], conflictID utxo.OutputID) {
	conflict := getSingleConflict(conflictIDs, conflictAlias)
	mesh.Ledger.ConflictDAG.CreateConflict(conflict, parentConflictIDs, set.NewAdvancedSet(conflictID))

...
func (s *Solidifier) Setup() {
	s.mesh.Storage.Events.BlockStored.Hook(event.NewClosure(func(event *BlockStoredEvent) {
...
func (s *Solidifier) Solidify(blockID BlockID) {
	s.mesh.Storage.Block(blockID).Consume(s.solidify)
}
...
func (s *Solidifier) RetrieveMissingBlock(blockID BlockID) (blockWasMissing bool) {
	s.mesh.Storage.BlockMetadata(blockID, func() *BlockMetadata {
		if cachedMissingBlock, stored := s.mesh.Storage.StoreMissingBlock(NewMissingBlock(blockID)); stored {
...
func (s *Solidifier) checkBlockSolidity(block *Block, blockMetadata *BlockMetadata) (blockBecameSolid bool) {
	...
	if !s.areParentBlocksValid(block) {
		if blockMetadata.SetObjectivelyInvalid(true) {
			s.mesh.Events.BlockInvalid.Trigger(&BlockInvalidEvent{BlockID: block.ID(), Error: ErrParentsInvalid})
		}
...
func (s *Solidifier) areParentBlocksValid(block *Block) (valid bool) {
	...
	block.ForEachParent(func(parent Parent) {
		...
		if parent.ID == EmptyBlockID {
			if s.mesh.Options.GenesisNode != nil {
				valid = *s.mesh.Options.GenesisNode == block.IssuerPublicKey()
				return
			}

			s.mesh.Storage.BlockMetadata(parent.ID).Consume(func(blockMetadata *BlockMetadata) {
		...
		s.mesh.Storage.Block(parent.ID).Consume(func(parentBlock *Block) {
...
func (s *Solidifier) isParentBlockValid(parentBlock *Block, childBlock *Block) (valid bool) {
	...
	s.mesh.Storage.BlockMetadata(parentBlock.ID()).Consume(func(blockMetadata *BlockMetadata) {

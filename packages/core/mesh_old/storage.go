...
func NewStorage(mesh *Mesh) (storage *Storage) {
	...
	storage = &Storage{
		...
		childStorage:                        objectstorage.NewStructStorage[Child](objectstorage.NewStoreWithRealm(mesh.Options.Store, database.PrefixMesh, PrefixChildren), cacheProvider.CacheTime(cacheTime), objectstorage.PartitionKey(BlockIDLength, ChildTypeLength, BlockIDLength), objectstorage.LeakDetectionEnabled(false), objectstorage.StoreOnCreation(true)),
		missingBlockStorage:                 objectstorage.NewStructStorage[MissingBlock](objectstorage.NewStoreWithRealm(mesh.Options.Store, database.PrefixMesh, PrefixMissingBlock), cacheProvider.CacheTime(cacheTime), objectstorage.LeakDetectionEnabled(false), objectstorage.StoreOnCreation(true)),
		attachmentStorage:                   objectstorage.NewStructStorage[Attachment](objectstorage.NewStoreWithRealm(mesh.Options.Store, database.PrefixMesh, PrefixAttachments), cacheProvider.CacheTime(cacheTime), objectstorage.PartitionKey(new(Attachment).KeyPartitions()...), objectstorage.LeakDetectionEnabled(false), objectstorage.StoreOnCreation(true)),
		markerIndexConflictIDMappingStorage: objectstorage.NewStructStorage[MarkerIndexConflictIDMapping](objectstorage.NewStoreWithRealm(mesh.Options.Store, database.PrefixMesh, PrefixMarkerConflictIDMapping), cacheProvider.CacheTime(cacheTime), objectstorage.LeakDetectionEnabled(false)),
		conflictVotersStorage:               objectstorage.NewStructStorage[ConflictVoters](objectstorage.NewStoreWithRealm(mesh.Options.Store, database.PrefixMesh, PrefixConflictVoters), cacheProvider.CacheTime(approvalWeightCacheTime), objectstorage.LeakDetectionEnabled(false)),
		latestConflictVotesStorage:          objectstorage.NewStructStorage[LatestConflictVotes](objectstorage.NewStoreWithRealm(mesh.Options.Store, database.PrefixMesh, PrefixLatestConflictVotes), cacheProvider.CacheTime(approvalWeightCacheTime), objectstorage.LeakDetectionEnabled(false)),
		latestMarkerVotesStorage:            objectstorage.NewStructStorage[LatestMarkerVotes](objectstorage.NewStoreWithRealm(mesh.Options.Store, database.PrefixMesh, PrefixLatestMarkerVotes), cacheProvider.CacheTime(approvalWeightCacheTime), LatestMarkerVotesKeyPartition, objectstorage.LeakDetectionEnabled(false)),
		conflictWeightStorage:               objectstorage.NewStructStorage[ConflictWeight](objectstorage.NewStoreWithRealm(mesh.Options.Store, database.PrefixMesh, PrefixConflictWeight), cacheProvider.CacheTime(approvalWeightCacheTime), objectstorage.LeakDetectionEnabled(false)),
		markerBlockMappingStorage:           objectstorage.NewStructStorage[MarkerBlockMapping](objectstorage.NewStoreWithRealm(mesh.Options.Store, database.PrefixMesh, PrefixMarkerBlockMapping), cacheProvider.CacheTime(cacheTime), MarkerBlockMappingPartitionKeys, objectstorage.StoreOnCreation(true)),
...
func (s *Storage) Setup() {
	s.mesh.Parser.Events.BlockParsed.Hook(event.NewClosure(func(event *BlockParsedEvent) {
		s.mesh.Storage.StoreBlock(event.Block)
	}))
}
...
func (s *Storage) StoreBlock(block *Block) {
	...
	if s.missingBlockStorage.DeleteIfPresent(blockID.Bytes()) {
		s.mesh.Storage.Events.MissingBlockStored.Trigger(&MissingBlockStoredEvent{blockID})
	}

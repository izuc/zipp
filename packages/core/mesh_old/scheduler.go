...
func NewScheduler(mesh *Mesh) *Scheduler {
	if mesh.Options.SchedulerParams.AccessManaMapRetrieverFunc == nil || mesh.Options.SchedulerParams.TotalAccessManaRetrieveFunc == nil {
	...
	maxBuffer := mesh.Options.SchedulerParams.MaxBufferSize

	// total supply of mana
	totalSupply := mesh.Options.SchedulerParams.TotalSupply

	// threshold after which confirmed blocks are not scheduled
	confirmedBlockScheduleThreshold := mesh.Options.SchedulerParams.ConfirmedBlockScheduleThreshold
	...
	accessManaCache := schedulerutils.NewAccessManaCache(
		mesh.Options.SchedulerParams.AccessManaMapRetrieverFunc,
		mesh.Options.SchedulerParams.TotalAccessManaRetrieveFunc,
		MinMana,
	)
	...
	return &Scheduler{
		...
		rate:                  atomic.NewDuration(mesh.Options.SchedulerParams.Rate),
		ticker:                time.NewTicker(mesh.Options.SchedulerParams.Rate),
...
func (s *Scheduler) Setup() {
	...
	s.mesh.ApprovalWeightManager.Events.BlockProcessed.Attach(event.NewClosure(func(event *BlockProcessedEvent) {
	...
	s.mesh.Scheduler.Events.BlockScheduled.Hook(event.NewClosure(func(event *BlockScheduledEvent) {
	...
	onBlockConfirmed := func(block *Block) {
		blockID := block.ID()
		var scheduled bool
		s.mesh.Storage.BlockMetadata(block.ID()).Consume(func(blockMetadata *BlockMetadata) {
			scheduled = blockMetadata.Scheduled()
		})
		if scheduled {
			return
		}
		s.mesh.Storage.Block(blockID).Consume(func(block *Block) {
	...
	s.mesh.ConfirmationOracle.Events().BlockAccepted.Attach(event.NewClosure(func(event *BlockAcceptedEvent) {
...
func (s *Scheduler) Submit(blockID BlockID) (err error) {
	if !s.mesh.Storage.Block(blockID).Consume(func(block *Block) {
		s.bufferMutex.Lock()
		defer s.bufferMutex.Unlock()
		err = s.submit(block)
	}) {
...
func (s *Scheduler) isEligible(blockID BlockID) (eligible bool) {
	s.mesh.Storage.BlockMetadata(blockID).Consume(func(blockMetadata *BlockMetadata) {
		eligible = blockMetadata.Scheduled() ||
			s.mesh.ConfirmationOracle.IsBlockConfirmed(blockID)
	})
	return
}
...
func (s *Scheduler) updateChildren(blockID BlockID) {
	s.mesh.Storage.Children(blockID).Consume(func(child *Child) {
...
func (s *Scheduler) submit(block *Block) error {
	...
	for _, droppedBlkID := range droppedBlockIDs {
		s.mesh.Storage.BlockMetadata(blockIDFromElementID(droppedBlkID)).Consume(func(blockMetadata *BlockMetadata) {
...
func (s *Scheduler) schedule() *Block {
	...
	for q := start; ; {
		...
		for blk != nil && !clock.SyncedTime().Before(blk.IssuingTime()) {
			...
			if s.mesh.ConfirmationOracle.IsBlockConfirmed(blkID) && clock.Since(blk.IssuingTime()) > s.confirmedBlkThreshold {
	...
	for {
		...
		case <-s.ticker.C:
			...
			if blk := s.schedule(); blk != nil {
				s.mesh.Storage.BlockMetadata(blk.ID()).Consume(func(blockMetadata *BlockMetadata) {

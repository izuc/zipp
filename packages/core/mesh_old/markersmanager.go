...
// ConflictMarkersMapper is a Mesh component that takes care of managing the Markers which are used to infer structural
// information about the Mesh in an efficient way.
...
func NewConflictMarkersMapper(mesh *Mesh) (b *ConflictMarkersMapper) {
	b = &ConflictMarkersMapper{
		mesh:  mesh,
		Manager: markers.NewManager(markers.WithStore(mesh.Options.Store)),
	}
...
func (b *ConflictMarkersMapper) InheritStructureDetails(block *Block, structureDetails []*markers.StructureDetails) (newStructureDetails *markers.StructureDetails, newSequenceCreated bool) {
	...
	//	bufferUsedRatio := float64(b.mesh.Scheduler.BufferSize()) / float64(b.mesh.Scheduler.MaxBufferSize())
	//	nodeQueueRatio := float64(b.mesh.Scheduler.NodeQueueSize(nodeID)) / float64(b.mesh.Scheduler.BufferSize())
	...
	//	}
	//	return b.mesh.Options.IncreaseMarkersIndexCallback(sequenceID, currentHighestIndex)
	// })

	newStructureDetails, newSequenceCreated = b.Manager.InheritStructureDetails(structureDetails, b.mesh.Options.IncreaseMarkersIndexCallback)
...
func (b *ConflictMarkersMapper) BlockID(marker markers.Marker) (blockID BlockID) {
	b.mesh.Storage.MarkerBlockMapping(marker).Consume(func(markerBlockMapping *MarkerBlockMapping) {
...
func (b *ConflictMarkersMapper) SetBlockID(marker markers.Marker, blockID BlockID) {
	b.mesh.Storage.StoreMarkerBlockMapping(NewMarkerBlockMapping(marker, blockID))
}
...
func (b *ConflictMarkersMapper) SetConflictIDs(marker markers.Marker, conflictIDs *set.AdvancedSet[utxo.TransactionID]) (updated bool) {
	...
	b.setConflictIDMapping(marker, b.mesh.Ledger.ConflictDAG.UnconfirmedConflicts(conflictIDs))
...
func (b *ConflictMarkersMapper) ConflictIDs(marker markers.Marker) (conflictIDs *set.AdvancedSet[utxo.TransactionID]) {
	b.mesh.Storage.MarkerIndexConflictIDMapping(marker.SequenceID()).Consume(func(markerIndexConflictIDMapping *MarkerIndexConflictIDMapping) {
...
func (b *ConflictMarkersMapper) setConflictIDMapping(marker markers.Marker, conflictIDs *set.AdvancedSet[utxo.TransactionID]) bool {
	return b.mesh.Storage.MarkerIndexConflictIDMapping(marker.SequenceID(), NewMarkerIndexConflictIDMapping).Consume(func(markerIndexConflictIDMapping *MarkerIndexConflictIDMapping) {
...
func (b *ConflictMarkersMapper) Floor(referenceMarker markers.Marker) (marker markers.Index, conflictIDs *set.AdvancedSet[utxo.TransactionID], exists bool) {
	b.mesh.Storage.MarkerIndexConflictIDMapping(referenceMarker.SequenceID(), NewMarkerIndexConflictIDMapping).Consume(func(markerIndexConflictIDMapping *MarkerIndexConflictIDMapping) {
...
// increaseMarkersIndexCallbackStrategy implements the default strategy for increasing marker Indexes in the Mesh.

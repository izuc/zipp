...
func (m *BlockTestFramework) PreventNewMarkers(enabled bool) *BlockTestFramework {
	if enabled && m.oldIncreaseIndexCallback == nil {
		m.oldIncreaseIndexCallback = m.mesh.Options.IncreaseMarkersIndexCallback
		m.mesh.Options.IncreaseMarkersIndexCallback = m.IncreaseMarkersIndexCallback
		return m
	}

	if !enabled && m.oldIncreaseIndexCallback != nil {
		m.mesh.Options.IncreaseMarkersIndexCallback = m.oldIncreaseIndexCallback
...
func (m *BlockTestFramework) LatestCommitment() (ecRecord *epoch.ECRecord, latestConfirmedEpoch epoch.Index, err error) {
	return m.mesh.Options.CommitmentFunc()
}
...
func (m *BlockTestFramework) IssueBlocks(blockAliases ...string) *BlockTestFramework {
	for _, blockAlias := range blockAliases {
		...
		event.Loop.Submit(func() {
			m.mesh.Storage.StoreBlock(m.blocksByAlias[currentBlockAlias])
		})
	}
...
func (m *BlockTestFramework) BlockMetadata(alias string) (blockMetadata *BlockMetadata) {
	m.mesh.Storage.BlockMetadata(m.blocksByAlias[alias].ID()).Consume(func(blkMetadata *BlockMetadata) {
...
func (m *BlockTestFramework) TransactionMetadata(blockAlias string) (txMeta *ledger.TransactionMetadata) {
	m.mesh.Ledger.Storage.CachedTransactionMetadata(m.TransactionID(blockAlias)).Consume(func(transactionMetadata *ledger.TransactionMetadata) {
...
func (m *BlockTestFramework) Transaction(blockAlias string) (tx utxo.Transaction) {
	m.mesh.Ledger.Storage.CachedTransaction(m.TransactionID(blockAlias)).Consume(func(transaction utxo.Transaction) {
		tx = transaction
	})
	return
}
...
func (m *BlockTestFramework) OutputMetadata(outputID utxo.OutputID) (outMeta *ledger.OutputMetadata) {
	m.mesh.Ledger.Storage.CachedOutputMetadata(outputID).Consume(func(outputMetadata *ledger.OutputMetadata) {
...
func (m *BlockTestFramework) Conflict(blockAlias string) (b *conflictdag.Conflict[utxo.TransactionID, utxo.OutputID]) {
	m.mesh.Ledger.ConflictDAG.Storage.CachedConflict(m.ConflictIDFromBlock(blockAlias)).Consume(func(conflict *conflictdag.Conflict[utxo.TransactionID, utxo.OutputID]) {
		b = conflict
	})
	return
}
...
func (m *BlockTestFramework) createGenesisOutputs() {
	...
	loadSnapshotToLedger(m.mesh.Ledger, m.snapshot)
}

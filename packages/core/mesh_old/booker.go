...
// Booker is a Mesh component that takes care of booking Blocks and Transactions by assigning them to the
...
func NewBooker(mesh *Mesh) (blockBooker *Booker) {
	blockBooker = &Booker{
		...
		mesh:              mesh,
		MarkersManager:      NewConflictMarkersMapper(mesh),
...
func (b *Booker) Setup() {
	b.mesh.Solidifier.Events.BlockSolid.Hook(event.NewClosure(func(event *BlockSolidEvent) {
		b.book(event.Block)
	}))

	b.mesh.Ledger.Events.TransactionConflictIDUpdated.Hook(event.NewClosure(func(event *ledger.TransactionConflictIDUpdatedEvent) {
		if err := b.PropagateForkedConflict(event.TransactionID, event.AddedConflictID, event.RemovedConflictIDs); err != nil {
			b.Events.Error.Trigger(errors.Errorf("failed to propagate Conflict update of %s to mesh: %w", event.TransactionID, err))
		}
	}))

	b.mesh.Ledger.Events.TransactionBooked.Attach(event.NewClosure(func(event *ledger.TransactionBookedEvent) {
		b.processBookedTransaction(event.TransactionID, BlockIDFromContext(event.Context))
	}))

	b.mesh.Booker.Events.BlockBooked.Attach(event.NewClosure(func(event *BlockBookedEvent) {
...
func (b *Booker) PayloadConflictIDs(blockID BlockID) (conflictIDs *set.AdvancedSet[utxo.TransactionID], err error) {
	...
	b.mesh.Storage.Block(blockID).Consume(func(block *Block) {
		transaction, isTransaction := block.Payload().(utxo.Transaction)
		if !isTransaction {
			return
		}

		b.mesh.Ledger.Storage.CachedTransactionMetadata(transaction.ID()).Consume(func(transactionMetadata *ledger.TransactionMetadata) {
			resolvedConflictIDs := b.mesh.Ledger.ConflictDAG.UnconfirmedConflicts(transactionMetadata.ConflictIDs())
...
func (b *Booker) book(block *Block) {
	...
	b.mesh.Storage.BlockMetadata(block.ID()).Consume(func(blockMetadata *BlockMetadata) {
...
func (b *Booker) bookTransaction(blockID BlockID, tx utxo.Transaction) (success bool) {
	if cachedAttachment, stored := b.mesh.Storage.StoreAttachment(tx.ID(), blockID); stored {
		cachedAttachment.Release()
	}

	if err := b.mesh.Ledger.StoreAndProcessTransaction(BlockIDToContext(context.Background(), blockID), tx); err != nil {
...
func (b *Booker) processBookedTransaction(id utxo.TransactionID, blockIDToIgnore BlockID) {
	b.mesh.Storage.Attachments(id).Consume(func(attachment *Attachment) {
		event.Loop.Submit(func() {
			b.mesh.Storage.Block(attachment.BlockID()).Consume(func(block *Block) {
				b.mesh.Storage.BlockMetadata(attachment.BlockID()).Consume(func(blockMetadata *BlockMetadata) {
...
func (b *Booker) propagateBooking(blockID BlockID) {
	b.mesh.Storage.Children(blockID).Consume(func(child *Child) {
		event.Loop.Submit(func() {
			b.mesh.Storage.Block(child.ChildBlockID()).Consume(func(approvingBlock *Block) {
...
func (b *Booker) readyForBooking(block *Block, blockMetadata *BlockMetadata) (ready bool) {
	...
	block.ForEachParent(func(parent Parent) {
		b.mesh.Storage.BlockMetadata(parent.ID).Consume(func(blockMetadata *BlockMetadata) {
...
// BookBlock tries to book the given Block (and potentially its contained Transaction) into the ledger and the Mesh.
...
func (b *Booker) determineBookingDetails(block *Block) (parentsStructureDetails []*markers.StructureDetails, parentsPastMarkersConflictIDs, inheritedConflictIDs *set.AdvancedSet[utxo.TransactionID], err error) {
	...
	inheritedConflictIDs.DeleteAll(b.mesh.Ledger.Utils.ConflictIDsInFutureCone(dislikedConflictIDs))

	return parentsStructureDetails, parentsPastMarkersConflictIDs, b.mesh.Ledger.ConflictDAG.UnconfirmedConflicts(inheritedConflictIDs), nil
}
...
func (b *Booker) blockBookingDetails(blockID BlockID) (structureDetails *markers.StructureDetails, pastMarkersConflictIDs, blockConflictIDs *set.AdvancedSet[utxo.TransactionID], err error) {
	...
	if !b.mesh.Storage.BlockMetadata(blockID).Consume(func(blockMetadata *BlockMetadata) {
...
func (b *Booker) collectShallowLikedParentsConflictIDs(block *Block) (collectedLikedConflictIDs, collectedDislikedConflictIDs *set.AdvancedSet[utxo.TransactionID], err error) {
	...
	block.ForEachParentByType(ShallowLikeParentType, func(parentBlockID BlockID) bool {
		if !b.mesh.Storage.Block(parentBlockID).Consume(func(block *Block) {
			transaction, isTransaction := block.Payload().(utxo.Transaction)
			if !isTransaction {
			...
			likedConflictIDs, likedConflictsErr := b.mesh.Ledger.Utils.TransactionConflictIDs(transaction.ID())
			...
			for it := b.mesh.Ledger.Utils.ConflictingTransactions(transaction.ID()).Iterator(); it.HasNext(); {
				conflictingTransactionID := it.Next()
				dislikedConflicts, dislikedConflictsErr := b.mesh.Ledger.Utils.TransactionConflictIDs(conflictingTransactionID)
...
func (b *Booker) collectWeakParentsConflictIDs(block *Block) (payloadConflictIDs *set.AdvancedSet[utxo.TransactionID], err error) {
	...
	block.ForEachParentByType(WeakParentType, func(parentBlockID BlockID) bool {
		if !b.mesh.Storage.Block(parentBlockID).Consume(func(block *Block) {
			...
			weakReferencePayloadConflict, weakReferenceErr := b.mesh.Ledger.Utils.TransactionConflictIDs(transaction.ID())
...
func (b *Booker) PropagateForkedConflict(transactionID utxo.TransactionID, addedConflictID utxo.TransactionID, removedConflictIDs *set.AdvancedSet[utxo.TransactionID]) (err error) {
	b.mesh.Utils.WalkBlockMetadata(func(blockMetadata *BlockMetadata, blockWalker *walker.Walker[BlockID]) {
		...
		for approvingBlockID := range b.mesh.Utils.ApprovingBlockIDs(blockMetadata.ID(), StrongChild) {
			blockWalker.Push(approvingBlockID)
		}
	}, b.mesh.Storage.AttachmentBlockIDs(transactionID), false)

...
func NewBlockFactory(mesh *Mesh, selector TipSelector, referencesFunc ...ReferencesFunc) *BlockFactory {
	sequence, err := kvstore.NewSequence(mesh.Options.Store, []byte(DBSequenceNumber), storeSequenceInterval)
	if err != nil {
		panic(fmt.Sprintf("could not create block sequence number: %v", err))
	}

	referenceProvider := NewReferenceProvider(mesh)
...
func (f *BlockFactory) issuePayload(p payload.Payload, references ParentBlockIDs, parentsCountOpt ...int) (*Block, error) {
	...
	epochCommitment, lastConfirmedEpochIndex, epochCommitmentErr := f.mesh.Options.CommitmentFunc()
...
func (f *BlockFactory) getIssuingTime(parents BlockIDs) time.Time {
	...
	for parent := range parents {
		f.mesh.Storage.Block(parent).Consume(func(blk *Block) {
...
func (f *BlockFactory) EarliestAttachment(transactionIDs utxo.TransactionIDs, earliestAttachmentMustBeBooked ...bool) (earliestAttachment *Block) {
	...
	for it := transactionIDs.Iterator(); it.HasNext(); {
		f.mesh.Storage.Attachments(it.Next()).Consume(func(attachment *Attachment) {
			f.mesh.Storage.Block(attachment.BlockID()).Consume(func(block *Block) {
				f.mesh.Storage.BlockMetadata(attachment.BlockID()).Consume(func(blockMetadata *BlockMetadata) {
...
func (f *BlockFactory) LatestAttachment(transactionID utxo.TransactionID) (latestAttachment *Block) {
	...
	f.mesh.Storage.Attachments(transactionID).Consume(func(attachment *Attachment) {
		f.mesh.Storage.Block(attachment.BlockID()).Consume(func(block *Block) {
			f.mesh.Storage.BlockMetadata(attachment.BlockID()).Consume(func(blockMetadata *BlockMetadata) {

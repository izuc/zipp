...
func TestTipManager_DataBlockTips(t *testing.T) {
	mesh := NewTestMesh()
	defer func(mesh *Mesh) {
		_ = mesh.Prune()
		mesh.Shutdown()
	}(mesh)
	mesh.Setup()
	tipManager := mesh.TipManager
	...
	// Block 1
	{
		blocks["1"] = createAndStoreParentsDataBlockInMasterConflict(mesh, NewBlockIDs(EmptyBlockID), NewBlockIDs())
		tipManager.AddTip(blocks["1"])
		mesh.TimeManager.updateTime(blocks["1"])
		mesh.TimeManager.updateSyncedState()
	...
	// Block 2
	{
		blocks["2"] = createAndStoreParentsDataBlockInMasterConflict(mesh, NewBlockIDs(EmptyBlockID), NewBlockIDs())
	...
	// Block 3
	{
		blocks["3"] = createAndStoreParentsDataBlockInMasterConflict(mesh, NewBlockIDs(blocks["1"].ID(), blocks["2"].ID()), NewBlockIDs())
	...
	// Add Block 4-8
	{
		...
		for count, n := range []int{4, 5, 6, 7, 8} {
			...
			blocks[nString] = createAndStoreParentsDataBlockInMasterConflict(mesh, NewBlockIDs(blocks["1"].ID()), NewBlockIDs())
...
// 	mesh.ConfirmationOracle = &MockConfirmationOracleTipManagerTest{confirmedBlockIDs: confirmedBlockIDs, confirmedMarkers: markers.NewMarkers(markers.NewMarker(0, 1))}
//
// 	testFramework := NewBlockTestFramework(mesh, WithGenesisOutput("G1", 5), WithGenesisOutput("G2", 8))
//
...
// 		// bookBlock(t, mesh, testFramework.Block("Block1"))
...
// 		assert.Equal(t, 1, tipManager.TipCount())
//
// 		// use this block to set MeshTime
// 		mesh.TimeManager.updateTime(testFramework.Block("Block2"))
// 	}
//
// 	// Block 3
// 	{
...
func TestTipManager_TimeSinceConfirmation_Unconfirmed(t *testing.T) {
	...
	mesh := NewTestMesh()
	mesh.Booker.MarkersManager.Manager = markers.NewManager(markers.WithCacheTime(0), markers.WithMaxPastMarkerDistance(10))
	defer mesh.Shutdown()
	...
	createTestMeshTSC(t, testFramework)
	...
	mesh.ConfirmationOracle = &MockConfirmationOracleTipManagerTest{confirmedBlockIDs: confirmedBlockIDs, confirmedMarkers: confirmedMarkers}
	mesh.TimeManager.updateTime(testFramework.Block("Marker-2/3"))
	mesh.TimeManager.updateSyncedState()
...
func TestTipManager_TimeSinceConfirmation_Confirmed(t *testing.T) {
	...
	mesh.ConfirmationOracle = confirmationOracle
...
func createTestMeshTSC(t *testing.T, testFramework *BlockTestFramework) {
...
func createAndStoreParentsDataBlockInMasterConflict(mesh *Mesh, strongParents, weakParents BlockIDs) (block *Block) {

...
func (t *TipManager) Setup() {
	t.mesh.Scheduler.Events.BlockScheduled.Attach(event.NewClosure(func(event *BlockScheduledEvent) {
		t.mesh.Storage.Block(event.BlockID).Consume(t.AddTip)
	}))
	...
	t.mesh.OrphanageManager.Events.BlockOrphaned.Hook(event.NewClosure(func(event *BlockOrphanedEvent) {
		t.deleteTip(event.Block.ID())
	}))

	t.mesh.TimeManager.Events.AcceptanceTimeUpdated.Attach(event.NewClosure(func(event *TimeUpdate) {
		t.tipsCleaner.RemoveBefore(event.UpdateTime.Add(-t.mesh.Options.TimeSinceConfirmationThreshold))
	}))

	t.mesh.OrphanageManager.Events.AllChildrenOrphaned.Hook(event.NewClosure(func(block *Block) {
...
func (t *TipManager) AddTip(block *Block) {
	...
	// skip removing tips if MeshWidth is enabled
	if t.TipCount() <= t.mesh.Options.MeshWidth {
		return
	}
...
func (t *TipManager) addTip(block *Block) (added bool) {
	...
	var invalid bool
	t.mesh.Storage.BlockMetadata(blockID).Consume(func(blockMetadata *BlockMetadata) {
...
func (t *TipManager) checkChildren(blockID BlockID) bool {
	...
	t.mesh.Storage.Children(blockID).Consume(func(child *Child) {
		if childScheduledConfirmed {
			return
		}

		childScheduledConfirmed = t.mesh.ConfirmationOracle.IsBlockConfirmed(child.ChildBlockID())
		if !childScheduledConfirmed {
			t.mesh.Storage.BlockMetadata(child.ChildBlockID()).Consume(func(blockMetadata *BlockMetadata) {
...
func (t *TipManager) isPastConeTimestampCorrect(blockID BlockID) (timestampValid bool) {
	...
	if t.mesh.TimeManager.LastAcceptedBlock().BlockID == EmptyBlockID {
		// if the genesis block is the last confirmed block, then there is no point in performing mesh walk
		// return true so that the network can start issuing blocks when the mesh starts
		return true
	}

	t.mesh.Storage.Block(blockID).Consume(func(block *Block) {
	...
	if t.mesh.ConfirmationOracle.IsBlockConfirmed(blockID) {
...
func (t *TipManager) checkMarker(marker markers.Marker, previousBlockID BlockID, blockWalker *walker.Walker[BlockID], markerWalker *walker.Walker[markers.Marker], minSupportedTimestamp time.Time) (blockID BlockID, timestampValid bool) {
	...
	t.mesh.Booker.MarkersManager.Manager.Sequence(marker.SequenceID()).Consume(func(sequence *markers.Sequence) {
		...
		if sequence.LowestIndex() < oldestUnconfirmedMarker.Index() {
			...
			if t.isMarkerOldAndConfirmed(markers.NewMarker(sequence.ID(), confirmedMarkerIdx), minSupportedTimestamp) {
				blockWalker.Push(t.mesh.Booker.MarkersManager.BlockID(oldestUnconfirmedMarker))
			}
		}
...
func (t *TipManager) isMarkerOldAndConfirmed(previousMarker markers.Marker, minSupportedTimestamp time.Time) bool {
	...
	if t.mesh.ConfirmationOracle.IsBlockConfirmed(referencedMarkerBlkID) && referenceMarkerBlkIssuingTime.Before(minSupportedTimestamp) {
		return true
	}
	return false
}
...
func (t *TipManager) getMarkerBlock(marker markers.Marker) (markerBlockID BlockID, markerBlockIssuingTime time.Time) {
	...
	blockID := t.mesh.Booker.MarkersManager.BlockID(marker)
...
func (t *TipManager) getOldestUnconfirmedMarker(pastMarker markers.Marker) markers.Marker {
	unconfirmedMarkerIdx := t.mesh.ConfirmationOracle.FirstUnconfirmedMarkerIndex(pastMarker.SequenceID())
	...
	for ; unconfirmedMarkerIdx <= pastMarker.Index(); unconfirmedMarkerIdx++ {
		...
		if t.mesh.Booker.MarkersManager.BlockID(currentMarker) == EmptyBlockID {
			continue
		}
		break
	}
...
func (t *TipManager) getPreviousConfirmedIndex(sequence *markers.Sequence, markerIndex markers.Index) markers.Index {
	...
	for ; sequence.LowestIndex() < markerIndex; markerIndex-- {
		...
		if blkID := t.mesh.Booker.MarkersManager.BlockID(currentMarker); blkID == EmptyBlockID || !t.mesh.ConfirmationOracle.IsBlockConfirmed(blkID) {
			continue
		}
		break
	}

...
type Manager struct {
	...
	seps            *shrinkingmap.ShrinkingMap[epoch.Index, map[mesh_old.BlockID]types.Empty]
	snapshotDepth   int
}
...
func NewManager(nmgr *notarization.Manager, depth int) (new *Manager) {
	new = &Manager{
		...
		seps:            shrinkingmap.New[epoch.Index, map[mesh_old.BlockID]types.Empty](),
		snapshotDepth:   depth,
	}
...
func (m *Manager) LoadSolidEntryPoints(seps *SolidEntryPoints) {
	...
	sep := make(map[mesh_old.BlockID]types.Empty)
...
func (m *Manager) InsertSolidEntryPoint(id mesh_old.BlockID) {
	m.Lock()
	defer m.Unlock()
...
func (m *Manager) RemoveSolidEntryPoint(b *mesh_old.Block) (err error) {
	m.Lock()
	defer m.Unlock()

...
func NewEpochCommitmentFactory(store kvstore.KVStore, mesh *mesh_old.Mesh, snapshotDepth int) *EpochCommitmentFactory {
...
func (f *EpochCommitmentFactory) ECRandRoots(ei epoch.Index) (ecr epoch.ECR, roots *epoch.CommitmentRoots, err error) {
	...
	return epoch.ComputeECR(roots.MeshRoot, roots.StateMutationRoot, roots.StateRoot, roots.ManaRoot), roots, nil
}
...
// insertMeshLeaf inserts blk to the Mesh sparse merkle tree.
func (f *EpochCommitmentFactory) insertMeshLeaf(ei epoch.Index, blkID mesh_old.BlockID) error {
	commitment, err := f.getCommitmentTrees(ei)
	if err != nil {
		return errors.Wrap(err, "could not get commitment while inserting mesh leaf")
	}
	return insertLeaf(commitment.meshTree, blkID.Bytes(), blkID.Bytes())
}

// removeMeshLeaf removes the block ID from the Mesh sparse merkle tree.
func (f *EpochCommitmentFactory) removeMeshLeaf(ei epoch.Index, blkID mesh_old.BlockID) error {
	commitment, err := f.getCommitmentTrees(ei)
	if err != nil {
		return errors.Wrap(err, "could not get commitment while deleting mesh leaf")
	}
	return removeLeaf(commitment.meshTree, blkID.Bytes())
}

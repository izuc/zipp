...
type Events struct {
	...
	// MeshTreeInserted is an event that gets triggered when a Block is inserted into the Mesh smt.
	MeshTreeInserted *event.Event[*MeshTreeUpdatedEvent]
	// MeshTreeRemoved is an event that gets triggered when a Block is removed from Mesh smt.
	MeshTreeRemoved *event.Event[*MeshTreeUpdatedEvent]
...
// MeshTreeUpdatedEvent is a container that acts as a dictionary for the MeshTree inserted/removed event related parameters.
type MeshTreeUpdatedEvent struct {
	// EI is the index of the block.
	EI epoch.Index
	// BlockID is the blockID that inserted/removed to/from the mesh smt.
	BlockID mesh_old.BlockID
}
